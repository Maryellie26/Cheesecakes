...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_order'])) {
    ...
    if ($cust_name === '' || $email === '' || $total <= 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO orders (customer_name, email, phone, address, subtotal, shipping_fee, total_amount, order_items, status) VALUES (?, ?, ?, ?, ?, 0.00, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("ssssddss", $cust_name, $email, $phone, $address, $total, $total, $order_json, $status);
            if ($stmt->execute()) {
        ...
        <div class="admin-card">
            <div class="top-flex">
                <div>
                    ...
                    <p style="color: #6d6260; font-size: 13.5px;">Track incoming orders, change shipment status, and review customer invoices.</p>
                </div>
